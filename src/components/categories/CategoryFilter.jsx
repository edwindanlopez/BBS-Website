      ...
      {allCategories.map((cat, i) => (
        ...
        <span key={i}>
          <Link to={`/category/${cat.fieldValue}/`}>
            <div
              css={[
                tw`w-12 h-12 rounded-full bg-pasteleGreen m-auto p-2 flex justify-center items-center hover:(ring-2 ring-orangeAmber)`,
                cat.fieldValue === category && tw`ring-2 ring-orangeAmber`,
              ]}
            >
              {(() => {
                if (cat.fieldValue === 'custom') {
                  return (
                    <img src={customIcon} alt="Custom drawer icon" tw="w-3/4" />
                  );
                }
                if (cat.fieldValue === 'closet') {
                  return <img src={closetIcon} alt="Closet icon" tw="w-3/4" />;
                }
                if (cat.fieldValue === 'restoration') {
                  return (
                    <img
                      src={restorationIcon}
                ...
                if (cat.fieldValue === 'bathroom') {
                  return (
                    <img src={bathroomIcon} alt="Bathroom icon" tw="w-3/4" />
                  );
                }
                if (cat.fieldValue === 'artistic') {
                  return (
                    <img
                      src={artisticIcon}
